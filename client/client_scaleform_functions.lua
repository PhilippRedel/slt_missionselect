...
function Scaleform.CreateInstructionalButton(scaleform, id, pad_index, button, text)
    BeginScaleformMovieMethod(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(id)

    if button ~= -1 then
        ScaleformMovieMethodAddParamPlayerNameString(GetControlInstructionalButton(pad_index, button, true))
    end
